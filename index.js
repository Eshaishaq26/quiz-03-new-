...
app.patch("/blogs/:id",(req,res)=>{
  ...
  // Update the specific blog with the matching id
  const addblog=blogs.find(blogs=>blogs = blogs.blogs.id === id) 
  addblog.title= title
  addblog.description= description
  addblog.image= image
    // Redirect to the /blogs route
  res.redirect("/blogs")
})
...
app.delete("/blogs/:id",(req,res)=>{
  ...
  //Quiz Write the code todelete the array element by youself
    // Find the index of the blogs with matching element.
    const index = blogs.findIndex(blogs => blogs.id === id);

    // If the blog is found, remove it from the array
  if (index !== -1) {
